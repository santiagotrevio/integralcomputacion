...
const dbPath = process.env.DB_PATH || path.resolve(__dirname, '../inventario.db');
const db = new sqlite3.Database(dbPath, (err) => {
    if (err) console.error('❌ Error al abrir DB:', err.message);
    else console.log(`📦 Base de datos: ${dbPath}`);
});
...
async function runMigrations() {
    ...
    return new Promise((resolve) => {
        const essentialColumns = [
            ...
            // quotes — columnas que pueden faltar en BDs antiguas
            "ALTER TABLE quotes ADD COLUMN id INTEGER",
            "ALTER TABLE quotes ADD COLUMN client_email TEXT DEFAULT ''",
            "ALTER TABLE quotes ADD COLUMN client_phone TEXT DEFAULT ''",
            "ALTER TABLE quotes ADD COLUMN client_company TEXT DEFAULT ''",
            "ALTER TABLE quotes ADD COLUMN notes TEXT DEFAULT ''",
            "ALTER TABLE quotes ADD COLUMN validity INTEGER DEFAULT 15",
            "ALTER TABLE quotes ADD COLUMN status TEXT DEFAULT 'active'",
            // clients — columnas que pueden faltar
            "ALTER TABLE clients ADD COLUMN company TEXT DEFAULT ''",
            "ALTER TABLE clients ADD COLUMN last_quoted_at DATETIME"
        ];
